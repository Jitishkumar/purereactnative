...
const PostItem = ({ post, onOptionsPress }) => {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        if (videoRefValue && duration > 0) {
          const newPosition = newProgress * duration;
          videoRefValue.seek(newPosition);
      ...
      onPanResponderRelease: async (evt, gestureState) => {
        ...
        setPlaying(true);
        // No need to call playAsync, setting playing state will control video
  ...
  const handleSeek = async (value) => {
    ...
    if (videoRefValue && duration > 0) {
      ...
      await videoRefValue.seek(newPosition);
      setCurrentTime(newPosition);
      setProgress(value);
    }
  };
  ...
  useEffect(() => {
    if (activeVideoId && activeVideoId !== post.id && playing) {
      ...
      setPlaying(false);
      // No need to call pauseAsync, setting playing state will control video
    }
  ...
  useEffect(() => {
    ...
    return () => {
      // No need to call pauseAsync, React will handle cleanup
    };
  }, [post.id, activeVideoId, isFullscreenMode, playing, post.type]);
  ...
  // Function to show likes modal with user list
