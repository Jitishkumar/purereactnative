...
import { View, Text, StyleSheet, Image, TouchableOpacity, Alert, Modal, ActivityIndicator, FlatList } from 'react-native';
...
// ConnectionsModal component - moved outside of ProfileScreen to fix ESLint warning
const ConnectionsModal = ({ visible, onClose, title, connections, isLoading, navigation }) => {
  return (
    <Modal
      visible={visible}
      transparent={true}
...
const AccountSwitcherModal = ({ visible, onClose, loadUserProfile }) => {
  ...
  useEffect(() => {
    ...
    // refreshAccountData is defined inside the component and doesn't depend on props or state
...
const ProfileScreen = () => {
  // Not using accounts and addAccount in this component
  const { } = useAccounts();
          ...
          <View style={styles.textPostContent}>
            ...
            <View style={styles.gridItemFooter}>
              <View style={styles.gridItemStats}>
                ...
                <Ionicons name="chatbubble" size={14} color="#ff00ff" style={styles.chatIcon} />
      ...
      >
        ...
        <View style={styles.gridItemOverlay}>
          <View style={styles.gridItemStats}>
            ...
            <Text style={styles.gridItemStatsText}>{item.likes?.[0]?.count || 0}</Text>
            <Ionicons name="chatbubble" size={14} color="#fff" style={styles.chatIcon} />
            <Text style={styles.gridItemStatsText}>{item.comments?.[0]?.count || 0}</Text>
          </View>
        </View>
      </TouchableOpacity>
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      fetchShortsCount();
      fetchUserContent();
      // fetchUserContent is defined inside the component and doesn't depend on props or state
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [activeTab])
  );
