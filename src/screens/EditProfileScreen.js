...
const EditProfileScreen = ({ navigation }) => {
  ...
  // const blinkAnimation = useRef(new Animated.Value(0)).current; // Removed unused animation
  ...
  const pickImage = async (type) => {
    try {
      ...
      const mediaType = type === 'cover' ? 'mixed' : 'photo';

      const options = {
        mediaType,
        includeBase64: true,
        quality: 0.5,
        maxVideoLength: 30, // Limit video duration to 30 seconds
        selectionLimit: 1,
      };
      ...
      if (!result.didCancel && result.assets && result.assets[0]) {
        ...
        const isVideo = file.type === 'video/mp4' || file.uri?.endsWith('.mp4');
      ...
      <TouchableOpacity onPress={() => pickImage('cover')} style={styles.coverContainer}>
        {coverUrl ? (
          ...
          <View style={styles.coverPlaceholder}>
            ...
            <Ionicons name="videocam-outline" size={40} color="#666" style={styles.iconSpacing} />
