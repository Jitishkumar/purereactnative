...
const ConfessionScreen = () => {
  ...
  // Removed unused state: const [mode, setMode] = useState('view');
  ...
  // We need to keep setLocationPermission as it's used in the geolocation callbacks
  ...
  useEffect(() => {
    (async () => {
      // Using React Native Geolocation instead of Expo's Location
  ...
  // searchTimeout state is now declared at the top of the component
  ...
  const pickImage = async () => {
    ...
    if (!result.didCancel && result.assets && result.assets.length > 0) {
      ...
      setMedia([...media, {
        ...
        name: asset.fileName || `${Math.random().toString(36).substring(2)}.jpg`,
      }]);
    }
  } catch (error) {
  ...
  // Mode selection component - not currently used
  /*
